...
export const getOrCreateHostingConfig = async ():
    ...
    console.error(`Could not find subdomain ${err}`);
    return null;
    }
}

export const uploadImagetoHoisting = async ({hosting, url, projectId, label}:
    StoreHostedImageParams): Promise<HostedAsset | null> =>{
        ...
        try{
            ...
            console.warn(`Could not find hosting url ${url}`);
            return null;
        }
